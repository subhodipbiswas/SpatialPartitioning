...
class REGAL:
    ...
    Biswas S, Chen F, Chen Z, Sistrunk A, Self N, Lu CT, Ramakrishnan N. REGAL: A districtalization framework for school
...
class SHC(REGAL):
    ...
    def search(self, run, argv):
        ...
        ''' 1. A plan/ zoning of N areas grouped into K districts, K < N'''
        initial = copy.deepcopy(self.solution)
        districts, district_ids, best_fval = self.solution['districts'], self.solution['district_ids'], self.solution['FuncVal']
        print('Run : {} Init Funcval: {:.4f}'.format(run, best_fval))

        args = list(self.args)
        args.append(districts)
        args.append(district_ids)
        ...
        while condition:
            ...
            while len(district_list) > 0:
                ...
                ''' 4. Identify a set of districts bordering on areas of district K that could be moved into district K without
                 destroying the internal contiguity of the donor district(s). '''
                areas = util.get_neighbors(recipient, self.args)

                '''5. Randomly select districts from this list until either there is a local improvement in the current
                ...
                while improve:
                    ...
                    while not improve and len(areas) > 0:
                        # Search the local solutions of 'recipient' district
                        area = areas[random.randrange(len(areas))]
                        donor = district_ids[area]  # Define the source cluster
            ...
            # Updates
            self.iter += 1
            fval = obj_func(districts.keys(), districts)
        ...
        alg_params = util.get_params(w1=self.args[6]['weight'],
                                     w2=1-self.args[6]['weight'],
                                     Iter=self.iter,
                                     MaxIter=self.max_iter)
        prop, info = util.get_alg_params(run, alg_params, self.iter, telapsed, termination, self.seed,
...
class SA(REGAL):
    """
    REGAL framework with Simulated Annealing as the local search algorithm.
    """
    ...
    def __init__(self, args=None, solution=None, seed=None):
        ...
        # Hyperparameters of Simulated Annealing
        self.T = 1
        self.T0 = 0
        self.a = 0.85
        self.minQ = 10
    ...
    def search(self, run, argv):
        ...
        while condition:
            ...
            while len(district_list) > 0:
                ...
                while improve:
                    ...
                    while not improve and len(areas) > 0:
                        # Randomly select districts until there is a local improvement
                        area = areas[random.randrange(len(areas))]
                        donor = district_ids[area]  # Define the source cluster
            ...
            self.iter += 1
            self.partitions.append(tuple(district_ids.values()))
        ...
        alg_params = util.get_params(w1=self.args[6]['weight'],
                                     w2=1-self.args[6]['weight'],
                                     Iter=self.iter, MaxIter=self.max_iter,
                                     T=self.T, T0=self.T0, a=self.a, minQ=self.minQ
                                     )
        prop, info = util.get_alg_params(run, alg_params, self.iter, telapsed, termination, self.seed,
                                         initialization, state,
                                         initial, final)
...
class TS(REGAL):
    ...
    def __init__(self, args=None, solution=None, seed=None):
        ...
        self.tabu_list_len = 80  # length of tabu list
        self.tabu_list = collections.deque([], self.tabu_list_len)
    ...
    def update_tabulist(self, moved):
        """Moving areas between districts and update tabu_list"""
    ...
    def find_best_move(self, district_list, util):

        district_ids = self.args[8]
        best_move = None
        best_diff = float("inf")
        improving_tabus = list()

        while district_list:
            # Pick a district and search for neighboring areas that can be swapped w/o breaking contiguity
            recipient = district_list[random.randrange(len(district_list))]
            district_list.remove(recipient)  # remove it so that new districts can be picked
            areas = util.get_neighbors(recipient, self.args)
    ...
    def search(self, run, argv):
        ...
        while condition:
            # Make a list of M districts
            district_list = [x for x in districts.keys()]
            random.shuffle(district_list)

            # Parallel search for best move
            best_move, best_diff, improving_tabus = self.find_best_move(district_list, util)
            moved = False
            ...
            if self.iter == 1 or self.iter % 5 == 0:
        ...
        alg_params = util.get_params(w1=self.args[6]['weight'],
                                     w2=1 - self.args[6]['weight'],
                                     Iter=self.iter, MaxIter=self.max_iter,
                                     TabuListLen=self.tabu_list_len
                                     )
        prop, info = util.get_alg_params(run, alg_params, self.iter, telapsed, termination, self.seed,
                                         initialization, state,
                                         initial, final)
